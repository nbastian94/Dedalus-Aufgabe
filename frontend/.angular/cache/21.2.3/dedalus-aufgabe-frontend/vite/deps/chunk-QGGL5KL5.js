  ...
  if (a(e)) {
    ...
    if (G(a2, k)) {
      let n = a2.replaceAll(k, (l2) => {
        ...
        return `var(${ae(r, re(c.join("-")))}${s2(i2) ? `, ${i2}` : ""})`;
      });
...
var b = { regex: { rules: { class: { pattern: /^\.([a-zA-Z][\w-]*)$/, resolve(e) {
  ...
  if (s2(t) && r.transform !== "strict") {
    let y = e.replace("-directive", ""), m2 = t, { colorScheme: R, extend: T, css: j } = m2, O = v(m2, ["colorScheme", "extend", "css"]), d = T || {}, { colorScheme: M } = d, z2 = v(d, ["colorScheme"]), u = R || {}, { dark: V } = u, L = v(u, ["dark"]), g = M || {}, { dark: te } = g, re2 = v(g, ["dark"]), K = s2(O) ? this._toVariables({ [y]: h(h({}, O), z2) }, r) : {}, A2 = s2(L) ? this._toVariables({ [y]: h(h({}, L), re2) }, r) : {}, X = s2(V) ? this._toVariables({ [y]: h(h({}, V), te) }, r) : {}, [G2, U] = [(f = K.declarations) != null ? f : "", K.tokens || []], [B, I] = [(x = A2.declarations) != null ? x : "", A2.tokens || []], [H2, W] = [(p = X.declarations) != null ? p : "", X.tokens || []], q2 = this.transformCSS(y, `${G2}${B}`, "light", "variable", r, i2, a2, n), Z = this.transformCSS(y, H2, "dark", "variable", r, i2, a2, n);
  ...
  if (s2(t)) {
    ...
    if (s3 !== "style") {
      ...
      t = r === "dark" ? c.reduce((m2, { type: d, selector: u }) => (s2(u) && (m2 += u.includes("[CSS]") ? u.replace("[CSS]", t) : this.getSelectorRule(u, l2, d, t)), m2), "") : C2(l2 != null ? l2 : ":root,:host", t);
    }
    if (o) {
      let c = { name: "primeui", order: "primeui" };
      i(o) && (c.name = m(o.name, { name: e, type: s3 })), s2(c.name) && (t = C2(`@layer ${c.name}`, t), a2 == null || a2.layerNames(c.name));
    }
    return t;
  }
  return "";
...
//# sourceMappingURL=chunk-QGGL5KL5.js.map
